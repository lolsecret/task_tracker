...
class TestRegistration:
    ...
    def test_username_max_length(self):
        register_request_data['username'] = 'lolopop12lolopop12'
        client = APIClient()
        response = client.post('/register/', register_request_data)
        assert response.status_code == 400
    ...
    def test_username_regex(self):
        register_request_data['username'] = 'tamer_sabirbek@gmail.com'
        client = APIClient()
        response = client.post('/register/', register_request_data)
        assert response.status_code == 400
...
class TestLogin:
    ...
    def test_can_login_with_email(self):
        client = APIClient()
        user = mixer.blend(User, username=login_request_data['username'], email='tamer_sabirbek@gmail.com')
        user.set_password(login_request_data['password'])
        user.save()
        login_request_data['username'] = 'tamer_sabirbek@gmail.com'
        response = client.post('/token/', login_request_data)
        assert response.status_code == 200
